...
class productController {
  ...
  getproduct(req, res, next) {
    ...
    res.render("products", { data: products, user: req.session.UserPassword }); // user is consistent now
    // res.sendFile(path.join(path.resolve(), "src", "views", "products.html"));
  }

  // Middleware to render newbook form
  getNweBook(req, res, next) {
    res.render("new-product-add", { errors: null, user: req.session.UserPassword });
  }
  ...
  // Middleware to update book page
  UpdatePage(req, res, next) {
    const { id } = req.params;
    let prduct_id = Model.getproductbtID(Number(id));
    if (Object.keys(prduct_id).length > 0) {
      res.render("update", { data: prduct_id, errors: null, user: req.session.UserPassword });
    } else {
      res.status(404).send(`
  ...
  delete_data(req, res, next) {
    ...
    res.redirect("/"); // when we delete we redirect to avoid errors
  }
